...
class TextCNN(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, dropout, kernel, num_filter, num_class, attention, contextualize):
        ...
        self.embed_drop = nn.Dropout(p=self.dropout)
        ...
        if self.contextualize:
            self.position = PositionalEncoding(self.embedding_dim, self.dropout)
            self.encoder_layer = nn.TransformerEncoderLayer(self.embedding_dim, 1)
    ...
    def forward(self, x, src_padding = None):
        ...
        if self.contextualize:
            x = self.position(x)
            x = self.encoder_layer(x.transpose(0, 1)).transpose(0, 1)
